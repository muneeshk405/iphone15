...
const Navbar = () => {
  return (
   <header className='w-full py-5 sm:px-10 px-5 flex justify-betwee items-center'>
    <nav className='flex w-full screen-max-width'>
      <img src={appleImg} alt="Apple" width={14} height={18} className='cursor-pointer' />
    <div className='flex flex-1 justify-center max-sm:hidden'>
      {
        navLists.map((item)=>{
          return <div key={item} className='px-5 text-sm cursor-pointer text-gray hover:text-white translate-all'>{item}</div>
        })
      }
